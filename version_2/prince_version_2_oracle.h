#include<stdio.h>
long circ_shift(long x,int pos){			/* right circular shifting the msg to 'pos' position */
...
long inv_SB(long msg){					/* takes the msg, extract nibble & substitute by inverse subbyte table and return 								   64 bit modified msg */
	long return_msg = 0;
	for(int i=0;i<16;i++){
		return_msg = return_msg<<4;
...
long MC(long msg){					/* storing the 64x64 matrix like each column is a 64 bit long int, store it in a 								   array M */
	char msg_bit;	long msg_,return_msg = 0;
	long M[64] = {	0x0888000000000000, 0x4044000000000000, 0x2202000000000000, 0x1110000000000000,
...
long SR(long msg){					/* storing shift row rule in an array, extracting nibble & checking where the msg 								   goes by the rule */
...
long inv_SR(long msg){					/* storing inverse shift row rule in an array, extracting nibble & checking where 								   the msg goes by the rule */
...
long oracle(long msg){	   			   	/* takes 64 bit msg, returns 64 bit cipher text */
	long k0 = 0x0123456789abcdef, k1 = 0xfedcba9876543210,
	rc[12] = {0x0000000000000000,0x13198a2e03707344,0xa4093822299f31d0,0x082efa98ec4e6c89,0x452821e638d01377,0xbe5466cf34e90c6c,0x7ef84f78fd955cb1,0x7aacf4538d971a60,0xc882d32f25323c54,0x9b8ded979cd838c7,0xd3b5a399ca0c2399,0x3f84d5b5b5470917};  

	msg = msg^k0;
	
	for(int i=0;i<11;i++){
		msg = (i%2 == 0) ? msg^k0^rc[i] : msg^k1^rc[i];
		msg = (i<5) ? SR(MC(SB(msg))) : (i == 5) ? inv_SB(rc[11]^k1^MC(k0^SB(msg))) : inv_SB(MC(inv_SR(msg)));
		}
